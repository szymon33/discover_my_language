...
describe GithubService::Repositories do
  describe '.list_user_repositories' do
    ...
    describe 'successful result', vcr: true do
      subject { client.list_user_repositories(user_name) }
      ...
      describe 'JSON' do
        subject { client.list_user_repositories(user_name).first }
    ...
    it 'failing result' do
      expect {
        client.list_user_repositories('Enough is enough!')
      }.to change {
        client.errors
      }.from([]).to(['Problem with connection'])
    end
  end
  ...
  describe '.list_languages', vcr: true do
    ...
    describe 'failing result' do
      subject do
        described_class.new.list_languages("I cycle every day - it's OK", repo)
      end
